...
/**
* phpBB Garage Custom Fields Class
* @package garage
*/
class custom_vehicle_fields
{
	...
	function get_option_lang($field_id, $lang_id, $field_type, $preview)
	{
		...
		else
		{
			$sql = 'SELECT option_id, value
				FROM ' . GARAGE_VEHICLE_FIELDS_LANG_TABLE . "
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if (($cp_result = $this->validate_profile_field($row['field_type'], $check_value, $row)) !== false)
			{
				...
				switch ($cp_result)
				{
					...
					case 'FIELD_REQUIRED':
						$error = sprintf($lang[$cp_result], $row['lang_name']);
						break;
					case 'FIELD_TOO_SHORT':
					case 'FIELD_TOO_SMALL':
						$error = sprintf($lang[$cp_result], $row['lang_name'], $row['field_minlen']);
						break;
					case 'FIELD_TOO_LONG':
					case 'FIELD_TOO_LARGE':
						$error = sprintf($lang[$cp_result], $row['lang_name'], $row['field_maxlen']);
						break;
					case 'FIELD_INVALID_CHARS':
						switch ($row['field_validation'])
						{
							case '[0-9]+':
								$error = sprintf($lang[$cp_result . '_NUMBERS_ONLY'], $row['lang_name']);
								break;
							case '[\w]+':
								$error = sprintf($lang[$cp_result . '_ALPHA_ONLY'], $row['lang_name']);
								break;
							case '[\w_\+\. \-\[\]]+':
								$error = sprintf($lang[$cp_result . '_SPACERS_ONLY'], $row['lang_name']);
								break;
						}
						break;
				}
	...
	*/
	function generate_profile_fields_template($mode, $user_id = 0, $profile_row = false)
	{
		...
		if ($mode == 'grab')
		{
			...
			$sql = 'SELECT *
				FROM ' . GARAGE_VEHICLE_FIELDS_DATA_TABLE . '
...
class custom_vehicle_fields_admin extends custom_vehicle_fields
{
	...
	function validate_options()
	{
		...
		$validate_options = '';
		foreach ($validate_ary as $option => $value)
		{
			$selected = ($this->vars['field_validation'] == $value) ? ' selected="selected"' : '';
			$validate_options .= '<option value="' . $value . '"' . $selected . '>' . $lang[$option] . '</option>';
		}
	...
	function get_string_options()
	{
		...
		$options = array(
			0 => array('TITLE' => $lang['FIELD_LENGTH'], 'FIELD' => '<input class="post" type="text" name="field_length" size="5" value="' . $this->vars['field_length'] . '" />'),
			1 => array('TITLE' => $lang['MIN_FIELD_CHARS'], 'FIELD' => '<input class="post" type="text" name="field_minlen" size="5" value="' . $this->vars['field_minlen'] . '" />'),
			2 => array('TITLE' => $lang['MAX_FIELD_CHARS'], 'FIELD' => '<input class="post" type="text" name="field_maxlen" size="5" value="' . $this->vars['field_maxlen'] . '" />'),
			3 => array('TITLE' => $lang['FIELD_VALIDATION'], 'FIELD' => '<select name="field_validation">' . $this->validate_options() . '</select>')
		);
	...
	function get_text_options()
	{
		...
		$options = array(
			0 => array('TITLE' => $lang['FIELD_LENGTH'], 'FIELD' => '<table border=0><tr><td><input name="rows" size="5" value="' . $this->vars['rows'] . '" class="post" /></td><td>[ ' . $lang['ROWS'] . ' ]</td></tr><tr><td><input name="columns" size="5" value="' . $this->vars['columns'] . '" class="post" /></td><td>[ ' . $lang['COLUMNS'] . ' ] <input type="hidden" name="field_length" value="' . $this->vars['field_length'] . '" /></td></tr></table>'),
			1 => array('TITLE' => $lang['MIN_FIELD_CHARS'], 'FIELD' => '<input class="post" type="text" name="field_minlen" size="10" value="' . $this->vars['field_minlen'] . '" />'),
			2 => array('TITLE' => $lang['MAX_FIELD_CHARS'], 'FIELD' => '<input class="post" type="text" name="field_maxlen" size="10" value="' . $this->vars['field_maxlen'] . '" />'),
			3 => array('TITLE' => $lang['FIELD_VALIDATION'], 'FIELD' => '<select name="field_validation">' . $this->validate_options() . '</select>')
		);
	...
	function get_int_options()
	{
		...
		$options = array(
			0 => array('TITLE' => $lang['FIELD_LENGTH'], 'FIELD' => '<input class="post" type="text" name="field_length" size="5" value="' . $this->vars['field_length'] . '" />'),
			1 => array('TITLE' => $lang['MIN_FIELD_NUMBER'], 'FIELD' => '<input class="post" type="text" name="field_minlen" size="5" value="' . $this->vars['field_minlen'] . '" />'),
			2 => array('TITLE' => $lang['MAX_FIELD_NUMBER'], 'FIELD' => '<input class="post" type="text" name="field_maxlen" size="5" value="' . $this->vars['field_maxlen'] . '" />'),
			3 => array('TITLE' => $lang['DEFAULT_VALUE'], 'FIELD' => '<input class="post" type="post" name="field_default_value" value="' . $this->vars['field_default_value'] . '" />')
		);
	...
	function get_bool_options()
	{
		...
		$options = array(
			0 => array('TITLE' => $lang['FIELD_TYPE'], 'EXPLAIN' => $lang['BOOL_TYPE_EXPLAIN'], 'FIELD' => '<input type="radio" name="field_length" value="1"' . (($this->vars['field_length'] == 1) ? ' checked="checked"' : '') . ' />' . $lang['RADIO_BUTTONS'] . '&nbsp; &nbsp;<input type="radio" name="field_length" value="2"' . (($this->vars['field_length'] == 2) ? ' checked="checked"' : '') . ' />' . $lang['CHECKBOX'] . '&nbsp; &nbsp;'),
			1 => array('TITLE' => $lang['DEFAULT_VALUE'], 'FIELD' => $this->generate_bool($profile_row, true))
		);
	...
	function get_dropdown_options()
	{
		...
		$options = array(
			0 => array('TITLE' => $lang['DEFAULT_VALUE'], 'FIELD' => $this->generate_dropdown($profile_row[0], true)),
			1 => array('TITLE' => $lang['NO_VALUE_OPTION'], 'EXPLAIN' => $lang['NO_VALUE_OPTION_EXPLAIN'], 'FIELD' => $this->generate_dropdown($profile_row[1], true))
		);
	...
	function get_date_options()
	{
		...
		$options = array(
			0 => array('TITLE' => $lang['DEFAULT_VALUE'], 'FIELD' => $this->generate_date($profile_row, true) . '<br /><input type="checkbox" name="always_now"' . ((isset($_REQUEST['always_now']) || $this->vars['field_default_value'] == 'now') ? ' checked="checked"' : '') . ' />&nbsp; ' . $lang['ALWAYS_TODAY'])
		);
