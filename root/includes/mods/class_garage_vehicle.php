...
class garage_vehicle
{
	...
	*/
	function get_user_add_quota()
	{
		...
		else //It Appears Some Groups Have Private Permissions & Quotas We Will Need To Check Them
		{
	...
	*/
	...
	/**
	* Get top rated vehicle for specific month & year
	*
	*/
	function get_earliest_rating()
	{
		global $db;
	...
	* Get top rated vehicle for specific month & year
	*
	* @param int $month value for month JAN = 1, FEB = 2 ... etc
	* @param int $year four digit value representing year
	*
	*/
	function get_month_toprated_vehicle($month, $year)
	{
		global $db;
		...
		if (empty($month))
		{
			//We Add 1 To Month As Janurary is 0 For This Function
			$month = $time['tm_mon'] + 1;
		}

		$rate_start_date = mktime(0, 0, 0, $month, 1, $year);
		$rate_end_date = mktime(23, 59, 59, $month + 1, 0, $year);

		$sql = $db->sql_build_query('SELECT',
			array(
			'SELECT'	=> 'v.id as vehicle_id, SUM(r.rating)as total_ratings, u.username, v.made_year, mk.make, md.model, u.user_colour, u.user_id, images.*',
			'FROM'		=> array(
				GARAGE_VEHICLES_TABLE	=> 'v',
				GARAGE_RATINGS_TABLE	=> 'r',
			...
			'WHERE'	=> "v.pending = 0
				...
				AND (r.rate_date >= $rate_start_date AND r.rate_date <= $rate_end_date)
	...
	/**
	* Assign template variables to show top rated vehicle for specific month
	*
	* @param int $month value for month JAN = 1, FEB = 2 ... etc
	* @param int $year four digit value representing year
	*
	*/
	function show_month_toprated_vehicle($month = NULL, $year = NULL)
	{
		global $user, $template, $db, $SID, $lang, $phpEx, $phpbb_root_path, $garage_config, $board_config;

		$data = null;

		$vehicle_data = $this->get_month_toprated_vehicle($month, $year);
		...
		if (!empty($vehicle_data))
		{
			$template->assign_vars(array(
				'S_MONTH_TOPRATED'				=> true,
				'MONTH_TOPRATED_THUMB' 			=> $thumb_image,
				'MONTH_TOPRATED_VEHICLE'		=> $vehicle_data['vehicle'],
				'MONTH_TOPRATED_USERNAME' 		=> $vehicle_data['username'],
				'MONTH_TOPRATED_IMAGE_TITLE'		=> $vehicle_data['attach_file'],
				'MONTH_TOPRATED_U_VIEW_IMAGE'		=> append_sid("{$phpbb_root_path}garage/garage.$phpEx?mode=view_image&amp;image_id=".$vehicle_data['attach_id']),
				'MONTH_TOPRATED_U_VIEW_VEHICLE' 	=> append_sid("{$phpbb_root_path}garage/garage_vehicle.$phpEx?mode=view_vehicle&amp;VID=".$vehicle_data['vehicle_id']),
				'MONTH_TOPRATED_U_VIEW_PROFILE' 	=> append_sid("memberlist.$phpEx?mode=viewprofile&amp;u=".$vehicle_data['user_id']),
				'MONTH_TOPRATED_USERNAME_COLOUR'	=> get_username_string('colour', $vehicle_data['user_id'], $vehicle_data['username'], $vehicle_data['user_colour']),
				'MONTH_TOPRATED_RATING'				=> $vehicle_data['total_ratings'],
			));
		}
	}
	...
	*/
	...
	{
		global $user, $template, $db, $SID, $lang, $phpEx, $phpbb_root_path, $garage_config, $board_config, $phpbb_root_path;


		//Start To Build SQl For Selecting Featured Vehicle..We Will Extend This Array Based On User Options
		$sql_array = array(
			'SELECT'	=> 'v.id, v.made_year, vg.image_id, v.user_id, mk.make, md.model, images.attach_id, images.attach_hits, images.attach_thumb_location, u.username, images.attach_is_image, images.attach_location, COUNT(mods.id) AS mod_count, v.made_year, mk.make, md.model, images.attach_file, (SUM(mods.install_price) + SUM(mods.price)) AS money_spent, sum( r.rating ) AS rating, u.user_colour',
			'FROM'		=> array(
				GARAGE_VEHICLES_TABLE	=> 'v',
				GARAGE_MAKES_TABLE	=> 'mk',
				GARAGE_MODELS_TABLE	=> 'md',
		...
		if ($total_vehicles > 0 || (!empty($garage_config['featured_vehicle_from_block'])))
		{
			...
			if (!empty($vehicle_data))
			{
				$vehicle_data['vehicle'] = "{$vehicle_data['made_year']} {$vehicle_data['make']} {$vehicle_data['model']}";
			}
			$db->sql_freeresult($result);
			...
			$template->assign_vars(array(
				...
				'U_VIEW_IMAGE'		=> (!empty($vehicle_data['attach_id'])) ? append_sid($absolute_url."{$phpbb_root_path}garage/garage.$phpEx?mode=view_image&amp;image_id=".$vehicle_data['attach_id']) : '',
				'U_VIEW_VEHICLE' 	=> append_sid($absolute_url."{$phpbb_root_path}garage/garage_vehicle.$phpEx?mode=view_vehicle&amp;VID=".$vehicle_data['id']),
	...
	*/
	...
	{
		global $required_position, $user, $template, $db, $SID, $lang, $phpEx, $garage_config, $garage_vehicle, $board_config, $phpbb_root_path;
		...
		for ($i = 0; $i < count($vehicle_data); $i++)
	 	{
			$template->assign_block_vars($template_block_row, array(
				'U_COLUMN_1' 		=> append_sid("{$phpbb_root_path}garage/garage_vehicle.$phpEx", "mode=view_vehicle&amp;VID=" . $vehicle_data[$i]['id']),
	...
	*/
	...
	{
		global $required_position, $user, $template, $db, $SID, $lang, $phpEx, $garage_config, $board_config, $phpbb_root_path;
	...
	function display_vehicle($owned)
	{
		...
		if ($owned == 'NO')
		{
			...
			else
			{
				...
				else if (($rating['total'] > 0) && ($garage_config['rating_permanent']))
				{
					$template->assign_vars(array(
				...
				else if (($rating['total'] > 0) && (!$garage_config['rating_permanent']) && ($garage_config['rating_always_updateable']))
				{
			 		$garage_template->rating_dropdown('rating');
					$template->assign_vars(array(
						'S_DISPLAY_RATE'	=> true,
				...
				else if (($rating['total'] > 0) && (!$garage_config['rating_permanent']) && (!$garage_config['rating_always_updateable']) && ($rating['rate_date'] > $vehicle['date_updated']))
				{
					$template->assign_vars(array(
				...
				else if (($rating['total'] > 0) && (!$garage_config['rating_permanent']) && (!$garage_config['rating_always_updateable']) && ($row['rate_date'] < $vehicle['date_updated']))
				{
			 		$garage_template->rating_dropdown('rating');
					$template->assign_vars(array(
						'S_DISPLAY_RATE'	=> true,
		...
		for ($i = 0; $i < count($category_data); $i++)
		{
			...
			for ($j = 0; $j < count($modification_data); $j++)
			{
				$template->assign_block_vars('category.modification', array(
					'U_IMAGE'	=> ($modification_data[$j]['attach_id']) ? append_sid("{$phpbb_root_path}garage/garage.$phpEx", "mode=view_image&amp;image_id=". $modification_data[$j]['attach_id']) : '',
					...
					'MODIFICATION'	=> '<a href="' . append_sid("{$phpbb_root_path}garage/garage_modification.$phpEx?mode=view_modification&amp;VID=$vid&amp;MID=" . $modification_data[$j]['id']) . '">' . $modification_data[$j]['title'] . '</a>',
					'U_EDIT'	=> (($owned == 'YES') || ($owned == 'MODERATE')) ? append_sid("{$phpbb_root_path}garage/garage_modification.$phpEx?mode=edit_modification&amp;MID=". $modification_data[$j]['id'] . "&amp;VID=$vid") : '',
					'U_DELETE'	=> ((($owned == 'YES') || ($owned == 'MODERATE')) && ((($auth->acl_get('u_garage_delete_modification'))) || ($auth->acl_get('m_garage_delete')))) ? 'javascript:confirm_delete_mod(' . $vid . ',' . $modification_data[$j]['id'] . ')' : '')
				);
				...
				if ((($garage_config['enable_mod_gallery'] == 1) && ( $modification_data[$j]['attach_is_image'] )) && ($garage_config['gallery_limit'] >= $mod_images_found || !$garage_config['gallery_limit']))
				{
					$mod_images_displayed = $mod_images_found;
					//Do we have a thumbnail?  If so, our job is simple here :)
					if ((empty($modification_data[$j]['attach_thumb_location']) == false) && ($modification_data[$j]['attach_thumb_location'] != $modification_data[$j]['attach_location']))
					{
		...
		if (count($insurance_data) > 0)
		{
			...
			for ($i = 0; $i < count($insurance_data); $i++)
			{
				$template->assign_block_vars('insurance.premium', array(
					...
					'U_EDIT'		=> (($owned == 'YES') || ($owned == 'MODERATE')) ? append_sid("{$phpbb_root_path}garage/garage_premium.$phpEx?mode=edit_premium&amp;INS_ID=".$insurance_data[$i]['id']."&amp;VID=$vid") : '',
					'U_DELETE'		=> ((($owned == 'YES') || ($owned == 'MODERATE')) && ((($auth->acl_get('u_garage_delete_insurance'))) || ($auth->acl_get('m_garage_delete')))) ? 'javascript:confirm_delete_insurance(' . $vid . ',' . $insurance_data[$i]['id'] . ')' : '',
					'U_INSURER'		=> append_sid("{$phpbb_root_path}garage/garage.$phpEx", "mode=insurance_review&amp;business_id=".$insurance_data[$i]['business_id']),
				));
			}
		}
		...
		if (count($quartermile_data) > 0)
		{
			...
			for ($i = 0; $i < count($quartermile_data); $i++)
			{
				$template->assign_block_vars('quartermile.run', array(
					...
					'U_IMAGE'	=> ($quartermile_data[$i]['attach_id']) ? append_sid("{$phpbb_root_path}garage/garage.$phpEx", "mode=view_image&amp;image_id=". $quartermile_data[$i]['attach_id']) : '',
					'IMAGE'		=> $user->img('garage_img_attached', 'IMAGE_ATTACHED'),
					'U_QUART'	=> append_sid("{$phpbb_root_path}garage/garage_quartermile.$phpEx?mode=view_quartermile&amp;QMID=".$quartermile_data[$i]['id']."&amp;VID=$vid"),
					'U_EDIT'	=> (($owned == 'YES') || ($owned == 'MODERATE')) ? append_sid("{$phpbb_root_path}garage/garage_quartermile.$phpEx?mode=edit_quartermile&amp;QMID=".$quartermile_data[$i]['id']."&amp;VID=$vid") : '',
					'U_DELETE' 	=> ((($owned == 'YES') || ($owned == 'MODERATE')) && ((($auth->acl_get('u_garage_delete_quartermile'))) || ($auth->acl_get('m_garage_delete')))) ? 'javascript:confirm_delete_quartermile(' . $vid . ',' . $quartermile_data[$i]['id'] . ')' : ''
				));
				...
				//See If Mod Has An Image Attached And Display Gallery If Enabled & Below Limits
				if ((($garage_config['enable_quartermile_gallery'] == 1) && ($quartermile_data[$i]['attach_is_image'])) && ($garage_config['gallery_limit'] >= $quartermile_images_found || !$garage_config['gallery_limit']))
				{
					$quartermile_images_displayed = $quartermile_images_found;
					//Do we have a thumbnail?  If so, our job is simple here :)
					if ((empty($quartermile_data[$i]['attach_thumb_location']) == false) && ($quartermile_data[$i]['attach_thumb_location'] != $quartermile_data[$i]['attach_location']))
					{
			...
			$template->assign_block_vars('dynorun', array());
		 	for ($i = 0; $i < count($dynorun_data); $i++)
		 	{
				$template->assign_block_vars('dynorun.run', array(
					...
					'U_IMAGE'		=> ($dynorun_data[$i]['attach_id']) ? append_sid("{$phpbb_root_path}garage/garage.$phpEx", "mode=view_image&amp;image_id=". $dynorun_data[$i]['attach_id']) : '',
					'IMAGE'			=> $user->img('garage_img_attached', 'IMAGE_ATTACHED'),
					'U_BHP'			=> append_sid("{$phpbb_root_path}garage/garage_dynorun.$phpEx?mode=view_dynorun&amp;DID=".$dynorun_data[$i]['did']."&amp;VID=$vid"),
					'U_EDIT'		=> (($owned == 'YES') || ($owned == 'MODERATE')) ? append_sid("{$phpbb_root_path}garage/garage_dynorun.$phpEx?mode=edit_dynorun&amp;DID=".$dynorun_data[$i]['did']."&amp;VID=$vid") : '',
					'U_DELETE'		=> ((($owned == 'YES') || ($owned == 'MODERATE')) && ((($auth->acl_get('u_garage_delete_dynorun'))) || ($auth->acl_get('m_garage_delete')))) ? 'javascript:confirm_delete_dynorun(' . $vid . ',' . $dynorun_data[$i]['id'] . ')' : ''
				));
				...
				//See If Mod Has An Image Attached And Display Gallery If Enabled & Below Limits
				if ((($garage_config['enable_dynorun_gallery'] == 1) && ( $dynorun_data[$i]['attach_is_image'] )) && ($garage_config['gallery_limit'] >= $dynorun_images_found || !$garage_config['gallery_limit']))
				{
					$dynorun_images_displayed = $dynorun_images_found;
					//Do we have a thumbnail?  If so, our job is simple here :)
					if ((empty($dynorun_data[$i]['attach_thumb_location']) == false) && ($dynorun_data[$i]['attach_thumb_location'] != $dynorun_data[$i]['attach_location']))
					{
		...
		if ( count($lap_data) > 0 )
		{
		 	...
		 	for ($i = 0; $i < count($lap_data); $i++)
			{
				$template->assign_block_vars('tracktime.lap', array(
					...
					'IMAGE'		=> $user->img('garage_img_attached', 'IMAGE_ATTACHED'),
					'U_IMAGE'	=> ($lap_data[$i]['attach_id']) ? append_sid("{$phpbb_root_path}garage/garage.$phpEx", "mode=view_image&amp;image_id=". $lap_data[$i]['attach_id']) : '',
					'U_TRACK'	=> append_sid("{$phpbb_root_path}garage/garage_track.$phpEx?mode=view_track&amp;TID=".$lap_data[$i]['track_id']."&amp;VID=$vid"),
					'U_LAP'		=> append_sid("{$phpbb_root_path}garage/garage_track.$phpEx?mode=view_lap&amp;LID=".$lap_data[$i]['lid']."&amp;VID=$vid"),
					'U_EDIT'	=> (($owned == 'YES') || ($owned == 'MODERATE')) ? append_sid("{$phpbb_root_path}garage/garage_track.$phpEx?mode=edit_lap&amp;LID=".$lap_data[$i]['lid']."&amp;VID=$vid") : '',
					'U_DELETE' 	=> ((($owned == 'YES') || ($owned == 'MODERATE')) && ((($auth->acl_get('u_garage_delete_lap'))) || ($auth->acl_get('m_garage_delete')))) ? 'javascript:confirm_delete_lap(' . $vid . ',' . $lap_data[$i]['lid'] . ')' : ''
				));
				...
				//See If Mod Has An Image Attached And Display Gallery If Enabled & Below Limits
				if ((($garage_config['enable_lap_gallery'] == 1) && ( $lap_data[$i]['attach_is_image'] )) && ($garage_config['gallery_limit'] >= $lap_images_found || !$garage_config['gallery_limit']))
				{
					$lap_images_displayed = $lap_images_found;
					//Do we have a thumbnail?  If so, our job is simple here :)
					if ((empty($lap_data[$i]['attach_thumb_location']) == false) && ($lap_data[$i]['attach_thumb_location'] != $lap_data[$i]['attach_location']))
					{
		...
		if (count($service_data) > 0)
		{
		 	...
		 	for ($i = 0; $i < count($service_data); $i++)
			{
				$template->assign_block_vars('service_history.service', array(
					...
					'U_GARAGE'	=> append_sid("{$phpbb_root_path}garage/garage.$phpEx?mode=garage_review&amp;BID=".$service_data[$i]['garage_id']."&amp;VID=$vid"),
					'U_EDIT'	=> (($owned == 'YES') || ($owned == 'MODERATE')) ? append_sid("{$phpbb_root_path}garage/garage_service.$phpEx?mode=edit_service&amp;SVID=".$service_data[$i]['id']."&amp;VID=$vid") : '',
					'U_DELETE' 	=> ((($owned == 'YES') || ($owned == 'MODERATE')) && ((($auth->acl_get('u_garage_delete_lap'))) || ($auth->acl_get('m_garage_delete')))) ? 'javascript:confirm_delete_service(' . $vid . ',' . $service_data[$i]['id'] . ')' : ''
				));
			}
		}
		...
		$template->assign_vars(array(
			'U_DELETE_MODIFICATION'		=> append_sid("{$phpbb_root_path}garage/garage_modification.$phpEx?mode=delete_modification"),
			'U_DELETE_QUARTERMILE'		=> append_sid("{$phpbb_root_path}garage/garage_quartermile.$phpEx?mode=delete_quartermile"),
			'U_DELETE_PREMIUM'			=> append_sid("{$phpbb_root_path}garage/garage_premium.$phpEx?mode=delete_premium"),
			'U_DELETE_DYNORUN'			=> append_sid("{$phpbb_root_path}garage/garage_dynorun.$phpEx?mode=delete_dynorun"),
			'U_DELETE_LAP'				=> append_sid("{$phpbb_root_path}garage/garage_track.$phpEx?mode=delete_lap"),
			'U_DELETE_SERVICE'			=> append_sid("{$phpbb_root_path}garage/garage_service.$phpEx?mode=delete_service"),
			'U_DELETE_VEHICLE2'			=> append_sid("{$phpbb_root_path}garage/garage_vehicle.$phpEx?mode=delete_vehicle"),
			'U_DELETE_BLOG'				=> append_sid("{$phpbb_root_path}garage/garage_blog.$phpEx?mode=delete_blog"),
			'U_GUESTBOOK'				=> append_sid("{$phpbb_root_path}garage/garage_guestbook.$phpEx?mode=view_guestbook&amp;VID=$vid"),
			'U_PROFILE'					=> append_sid("memberlist.$phpEx?mode=viewprofile&amp;u=".$vehicle['user_id']),
			'U_VIEW_VEHICLE'			=> ($owned == 'YES' || $auth->acl_get('m_garage_edit')) ? append_sid("{$phpbb_root_path}garage/garage_vehicle.$phpEx?mode=view_vehicle&amp;VID=$vid") : '',
			'U_EDIT_VEHICLE'			=> ($owned == 'YES' || $auth->acl_get('m_garage_edit')) ? append_sid("{$phpbb_root_path}garage/garage_vehicle.$phpEx?mode=edit_vehicle&amp;VID=$vid") : '',
			'U_DELETE_VEHICLE'			=> (($owned == 'YES' && $auth->acl_get('u_garage_delete_vehicle') || $auth->acl_get('m_garage_delete')) || ($auth->acl_get('m_garage_delete'))) ? 'javascript:confirm_delete_vehicle(' . $vid . ')' : '',
			'U_ADD_MODIFICATION'		=> ($owned == 'YES' || $auth->acl_get('m_garage_edit')) ? append_sid("{$phpbb_root_path}garage/garage_modification.$phpEx?mode=add_modification&amp;VID=$vid") : '',
			'U_ADD_INSURANCE'			=> (($owned == 'YES' || $auth->acl_get('m_garage_edit')) && $garage_config['enable_insurance'] ) ? append_sid("{$phpbb_root_path}garage/garage_premium.$phpEx?mode=add_premium&amp;VID=$vid") : '',
			'U_ADD_QUARTERMILE'			=> (($owned == 'YES' || $auth->acl_get('m_garage_edit')) && $garage_config['enable_quartermile'] ) ? append_sid("{$phpbb_root_path}garage/garage_quartermile.$phpEx?mode=add_quartermile&amp;VID=$vid") : '',
			'U_ADD_DYNORUN'				=> (($owned == 'YES' || $auth->acl_get('m_garage_edit')) && $garage_config['enable_dynorun'] ) ? append_sid("{$phpbb_root_path}garage/garage_dynorun.$phpEx?mode=add_dynorun&amp;VID=$vid") : '',
			'U_ADD_LAP'					=> (($owned == 'YES' || $auth->acl_get('m_garage_edit')) && $garage_config['enable_tracktime'] ) ? append_sid("{$phpbb_root_path}garage/garage_track.$phpEx?mode=add_lap&amp;VID=$vid") : '',
			'U_ADD_SERVICE'				=> (($owned == 'YES' || $auth->acl_get('m_garage_edit')) && $garage_config['enable_service'] ) ? append_sid("{$phpbb_root_path}garage/garage_service.$phpEx?mode=add_service&amp;VID=$vid") : '',
			'U_MANAGE_VEHICLE_GALLERY'	=> ($owned == 'YES' || $auth->acl_get('m_garage_edit')) ? append_sid("{$phpbb_root_path}garage/garage_vehicle.$phpEx?mode=manage_vehicle_gallery&amp;VID=$vid") : '',
			'U_SET_MAIN_VEHICLE'		=> (($owned == 'YES' || $auth->acl_get('m_garage_edit')) && ($vehicle['main_vehicle'] == 0)) ?  append_sid("{$phpbb_root_path}garage/garage_vehicle.$phpEx?mode=set_main_vehicle&amp;VID=$vid"): '' ,
			'U_MODERATE_VEHICLE'		=> ($owned == 'NO' && $auth->acl_get('m_garage_delete')) ?  append_sid("{$phpbb_root_path}garage/garage_vehicle.$phpEx?mode=moderate_vehicle&amp;VID=$vid"): '' ,
			'U_HILITE_IMAGE'			=> (($vehicle['attach_id']) && ($vehicle['attach_is_image']) && (!empty($vehicle['attach_thumb_location'])) && (!empty($vehicle['attach_location']))) ?  append_sid("{$phpbb_root_path}garage/garage.$phpEx?mode=view_image&amp;image_id=". $vehicle['attach_id']): '' ,
			...
			'S_MODE_ACTION'				=> append_sid("{$phpbb_root_path}garage/garage_vehicle.$phpEx", "mode=rate_vehicle"),
			...
			'AVATAR_IMG' 			=> ($user->optionget('viewavatars')) ? get_user_avatar($vehicle['user_avatar'], $vehicle['user_avatar_type'], $vehicle['user_avatar_width'], $vehicle['user_avatar_height']) : '',
	...
	*/
	function get_pending_vehicles()
	{
		...
		$sql = $db->sql_build_query('SELECT',
			array(
			'SELECT'	=> "v.*, ROUND(v.weighted_rating, 2) as weighted_rating, i.*, mk.make, md.model, v.made_year, mk.make, md.model, u.username, u.user_avatar_type, u.user_avatar, u.user_id, u.user_colour",
			'FROM'		=> array(
				GARAGE_VEHICLES_TABLE	=> 'v',
				GARAGE_MAKES_TABLE	=> 'mk',
				GARAGE_MODELS_TABLE	=> 'md',
	...
	*/
	function get_vehicles_by_user($user_id)
	{
		...
		$sql = $db->sql_build_query('SELECT',
			array(
			'SELECT'	=> 'v.id, v.made_year, mk.make, md.model',
			'FROM'		=> array(
				GARAGE_VEHICLES_TABLE	=> 'v',
				GARAGE_MAKES_TABLE	=> 'mk',
				GARAGE_MODELS_TABLE	=> 'md',
	...
	*/
	function get_newest_vehicles($limit)
	{
		...
		$sql = $db->sql_build_query('SELECT',
			array(
			'SELECT'	=> 'v.id, v.made_year, mk.make, md.model, v.user_id, v.date_created AS POI, u.username, u.user_colour, u.user_id',
			'FROM'		=> array(
				GARAGE_VEHICLES_TABLE	=> 'v',
				GARAGE_MAKES_TABLE	=> 'mk',
				GARAGE_MODELS_TABLE	=> 'md',
	...
	*/
	function get_top_rated_vehicles($limit)
	{
		...
		$sql = $db->sql_build_query('SELECT',
			array(
			'SELECT'	=> 'v.id, v.user_id, ROUND(v.weighted_rating, 2) as weighted_rating, u.username, v.made_year, mk.make, md.model, u.user_colour, u.user_id',
			'FROM'		=> array(
				GARAGE_VEHICLES_TABLE	=> 'v',
				GARAGE_MAKES_TABLE		=> 'mk',
				GARAGE_MODELS_TABLE		=> 'md',
	...
	*/
	function get_most_viewed_vehicles($limit)
	{
		...
		$sql = $db->sql_build_query('SELECT',
			array(
			'SELECT'	=> 'v.id, v.made_year, mk.make, md.model, v.user_id, v.views AS POI, u.username, u.user_colour, u.user_id',
			'FROM'		=> array(
				GARAGE_VEHICLES_TABLE	=> 'v',
				GARAGE_MAKES_TABLE		=> 'mk',
				GARAGE_MODELS_TABLE		=> 'md',
	...
	*/
	function get_latest_updated_vehicles($limit)
	{
		...
		$sql = $db->sql_build_query('SELECT',
			array(
			'SELECT'	=> 'v.id, v.made_year, v.user_id, v.date_updated, u.username, v.made_year, mk.make, md.model, u.user_colour',
			'FROM'		=> array(
				GARAGE_VEHICLES_TABLE	=> 'v',
				GARAGE_MAKES_TABLE		=> 'mk',
				GARAGE_MODELS_TABLE		=> 'md',
	...
	*/
	function get_user_main_vehicle($user_id)
	{
		...
		$sql = $db->sql_build_query('SELECT',
			array(
			'SELECT'	=> 'g.*, images.*, mk.make, md.model, g.made_year, mk.make, md.model, count(mods.id) AS total_mods, ( SUM(mods.price) + SUM(mods.install_price)) AS total_spent, u.username, u.user_avatar_type, u.user_avatar, u.user_id, u.user_colour',
			'FROM'		=> array(
				GARAGE_VEHICLES_TABLE	=> 'g',
				GARAGE_MAKES_TABLE		=> 'mk',
				GARAGE_MODELS_TABLE		=> 'md',
	...
	*/
	function profile_integration($user_id)
	{
		...
		if (!empty($vehicle_data))
		{
			...
			if ($garage_config['profile_thumbs'] == 1)
			{
				...
				for ($i=0; $i < count($gallery_data); $i++)
		  		{
					if ($gallery_data[$i]['attach_is_image'])
		  			{
						// Do we have a thumbnail?  If so, our job is simple here :)
						if ((empty($gallery_data[$i]['attach_thumb_location']) == false) && ($gallery_data[$i]['attach_thumb_location'] != $gallery_data[$i]['attach_location']) && ( $vehicle_images_found <= 12))
						{
							// Form the image link
				...
				for ($i=0; $i < count($mod_data); $i++)
				{
					if ($mod_data[$i]['attach_is_image'])
				  	{
						// Do we have a thumbnail?  If so, our job is simple here :)
						if ((empty($mod_data[$i]['attach_thumb_location']) == false) && ($mod_data[$i]['attach_thumb_location'] != $mod_data[$i]['attach_location']) && ( $vehicle_images_found <= 12))
						{
							// Form the image link
			...
			{
				if ((!empty($vehicle_data['attach_thumb_location'])) && (!empty($vehicle_data['attach_location'])))
				{
			...
			$template->assign_vars(array(
				...
				'U_GARAGE_USER_SEARCH'	=> append_sid("{$phpbb_root_path}garage/garage.$phpEx?mode=search_results&amp;search_username=1&amp;username={$member['username']}"))
			);
